# Tailwind CSS 教科書 詳細目次（節レベル）

各章は「ねらい」→「節構成」→「主な参考資料（候補）」で示します。参考 URL は執筆時に WebFetch で実在確認のうえ各章末へ掲載します。記号の凡例: ◎=一次情報の核、○=補助。
...
- 28.1 主な批判: 「HTML が汚い」「インラインスタイルと同じ」「学習コスト」
- 28.2 それぞれへの作者・コミュニティの反論
- 28.3 関心の分離をめぐる長年の論争の整理
- 28.4 セマンティック CSS 派（BEM 等）との比較で見える得失
- 28.5 計測できる効果（CSS サイズ・変更容易性・開発速度）の実態
- 28.6 「向くプロジェクト・向かないプロジェクト」の結論
- ◎ Adam Wathan の思想記事 / ○ 著名な批判記事と反論
...
各章は STYLEGUIDE.md に従い、章末に WebFetch で確認済みの「参考資料」を付けます。
