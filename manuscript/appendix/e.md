...
> 「出力は <strong>React の関数コンポーネント（TypeScript, tsx）</strong>で、props で `intent`（primary/secondary）と `size`（sm/md）を受けられるようにしてください。バリアントは **CVA** で定義し、`cn` ヘルパーで `className` を合成してください。」
