...
<button class="<%= class_names(base, styles[intent || :primary]) %>">
  <%= label %>
</button>
```
...
partial が大きくなり、ロジックやバリアントが増えてくると、**ViewComponent** が活きてきます。これは「**コンポーネントを Ruby のクラス＋テンプレートの組**として定義する」公式エコシステムの仕組みで、再利用・テスト・カプセル化に優れます。
