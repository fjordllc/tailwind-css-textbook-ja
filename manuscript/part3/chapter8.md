...
整理すると、<strong>「`app/assets/tailwind/application.css` に書く → `bin/dev` が変更を監視して `app/assets/builds/` に出力 → Propshaft がそれを配信する」</strong>という一本道です。この流れさえ押さえれば、Rails での Tailwind は迷いません。
