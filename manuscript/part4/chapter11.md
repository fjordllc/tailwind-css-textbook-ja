...
文字は情報そのものです。読みやすさは、フォントサイズと<strong>行間（line-height）</strong>の組み合わせで決まります。素の CSS では `font-size` と `line-height` を別々に指定しますが、Tailwind では `text-*` ひとつで両方をいい感じに設定してくれます。
