...
ファイルをダブルクリックしてブラウザで開き、青い見出しとボタン（ホバーで色が濃くなる）が出れば成功です。**ただしこれは開発・学習用**です（本番では F.2 や F.3 を使います）。ブラウザだけで完結するので、`class` をいじって**結果がすぐ変わる**のを体感してください。
...
ブラウザで `http://localhost:3000` を開き、青い見出しが出れば成功です。試しに `text-blue-600` を `text-red-600` に変えて保存すると、ウォッチが効いて自動で赤くなります。これが「入力 CSS → ビルド → 配信」の流れ（第8章）を体感する最短ルートです。
...
- **何も効かない**: CSS エントリに `@import "tailwindcss";` があるか、それをページ/アプリが読み込んでいるか確認（第7章）。古い記事の `@tailwind base; ...` は v4 では動きません。
- **変更が反映されない**: ウォッチが動いているか（`bin/dev` や `npm run dev`）を確認（第7章）。
- **一部のクラスだけ効かない**: クラス名を `text-${color}` のように動的に組み立てていないか（第4・27章）。
