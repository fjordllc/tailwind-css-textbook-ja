...
保存済みの設定（`localStorage`）か OS 設定を見て、最初の描画より前にクラスを付けます。Rails でも React/Next.js でも、考え方は同じです。Rails ならレイアウトの `<head>` にこのスクリプトを直接書きます。Next.js（App Router）では、同様のスクリプトを `<head>` に同期実行で差し込みます（サーバー側ではユーザーのテーマを知れないため、この「描画前の小さなスクリプト」がどの環境でも定石になります）。
...
* Tailwind CSS Docs — Transform（translate / scale / rotate）: https://tailwindcss.com/docs/translate
...
- **base 戦略**: フォーム要素すべてにグローバルにリセットを当てる（クラス不要）。多くの場合こちらで十分です。
- **class 戦略**: `form-input`・`form-select`・`form-checkbox`・`form-radio`・`form-textarea` というクラスを生成し、付けた要素にだけリセットを当てる。既存の見た目を壊したくないとき向け。
...
ここで役立つのが第6章の `focus-visible:` です。これは「**キーボード操作でフォーカスしたとき**」だけ効き、マウスのクリック時には出ません。マウス利用者には余計な枠を出さず、キーボード利用者にはしっかり枠を見せる、という理想的な挙動です。
...
- **コントラスト比を確保する**: 文字と背景のコントラストが低いと、弱視の人や明るい屋外では読めません。`text-gray-400` を白背景に置くような薄い文字は要注意です。WCAG の基準（本文で 4.5:1 以上が目安）を満たす配色を選びます。
