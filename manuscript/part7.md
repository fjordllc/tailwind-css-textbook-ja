...
**判断基準**: 色・余白・フォントなど、**デザインの基幹となる値はテーマトークンに集約**する（第5章・第26章）。とくに色は、ダークモードのことを考えると直値を避けるべき筆頭です。
...
2. **直値・任意値をテーマに寄せる（27.1, 27.5）**: 繰り返し使われている直値を洗い出し、テーマトークンに昇格させる。
