...
[第4章](chapter4.md)の最後で、`p-6` が `calc(var(--spacing) * 6)` という CSS 変数を使った値になることを見ました。この章では、その `--spacing` をはじめとする<strong>テーマ（デザイントークン）</strong>の仕組みを掘り下げます。v4 で最も大きく変わった部分であり、Tailwind を「自分のプロジェクトの色や余白に合わせて使う」ための心臓部です。
