...
ここまでで、ユーティリティが生成される仕組み（[第4章](chapter4.md)）と、その値を決めるテーマ（[第5章](chapter5.md)）を見てきました。残るピースが**バリアント**です。`hover:bg-blue-700` の `hover:`、`md:flex` の `md:` のように、ユーティリティの前に付けて**「どんな条件のときに効かせるか」を指定する接頭辞**——これがバリアントです。[第3章](../part1/chapter3.md)で「Tailwind がインラインスタイルと違う最大の理由」として挙げた、状態とレスポンシブを支える仕組みです。
