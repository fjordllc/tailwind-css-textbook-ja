...
ここまでで、ユーティリティが生成される仕組み（[第4章](chapter4.md)）と、その値を決めるテーマ（[第5章](chapter5.md)）を見てきました。残るピースが**バリアント**です。`hover:bg-blue-700` の `hover:`、`md:flex` の `md:` のように、ユーティリティの前に付けて<strong>「どんな条件のときに効かせるか」を指定する接頭辞</strong>——これがバリアントです。[第3章](../part1/chapter3.md)で「Tailwind がインラインスタイルと違う最大の理由」として挙げた、状態とレスポンシブを支える仕組みです。
...
<figure>
<img src="../figures/fig-6-2.svg" alt="hover:bg-* と md:flex がそれぞれ :hover と @media に変換される図">
<figcaption>図 6-2　バリアントは、条件を CSS のセレクタに変えるための記法。</figcaption>
</figure>
...
- `focus-visible:` … キーボード操作などでフォーカスが当たったとき。マウスクリック時には出さない、アクセシビリティに配慮したフォーカス表示に使います（[第21章](../part5/chapter21.md)）
...
- **ブレークポイント**（`sm:` `md:` `lg:` `xl:` `2xl:`）… 画面幅に応じた切り替え（[第17章](../part5/chapter17.md)で詳述）
- `dark:` … ダークモードのとき（[第18章](../part5/chapter18.md)）
- `motion-reduce:` … ユーザーが「視差効果を減らす」設定にしているとき。過剰なアニメーションを抑えるために使います（[第19章](../part5/chapter19.md)・[第21章](../part5/chapter21.md)）
- `print:` … 印刷時
...
- `aria-*:` … `aria-checked` や `aria-pressed` などの ARIA 属性に反応。状態を見た目に反映できます（[第21章](../part5/chapter21.md)）
...
<figure>
<img src="../figures/fig-6-3.svg" alt="dark:md:hover: が外側から内側へ条件を絞る様子を示す図">
<figcaption>図 6-3　バリアントのスタック。`dark:md:hover:` は外から内へ条件を重ねる。</figcaption>
</figure>

## 6.8 バリアントのスタック（評価順）

バリアントは<strong>重ねがけ（スタック）</strong>できます。`dark:` かつ `md:` 以上かつ `hover:` のとき、というように条件を組み合わせられます。
...
複雑に見えますが、左から順に条件を絞り込んでいるだけです。なお、書く順番は読みやすさのために<strong>「環境 → 状態」の順（`dark:md:hover:`）に揃える</strong>チーム規約にしておくと、後から読みやすくなります。
...
こう書くと、`dark:` は「OS 設定」ではなく「`.dark` クラスが祖先に付いているとき」に効くようになります（[第18章](../part5/chapter18.md)で詳しく扱います）。

ここまでで、Tailwind の三本柱——**ユーティリティの生成（[第4章](chapter4.md)）・テーマ（[第5章](chapter5.md)）・バリアント（[第6章](chapter6.md)）**——がそろいました。この 3 つが分かれば、Tailwind の挙動はもう「魔法」ではありません。次の第3部では、これらを実際に動かすための**環境構築**に進みます。
