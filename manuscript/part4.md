...
**`space-x-*` / `space-y-*`**: 隣り合う子の間にマージンを入れます（最初の要素以外の片側にマージンを付ける仕組み）。
...
線形だけでなく、放射状（`bg-radial-*`）や円錐（`bg-conic-*`）も使えます。装飾過多にならない範囲で、ヒーロー領域やボタンのアクセントに使うとよいでしょう。
...
> v4 の重要な注意: 影のスケール名が v3 から**ずれました**。v3 の `shadow-sm` は v4 の `shadow-xs` に、v3 の `shadow`（無印）は v4 の `shadow-sm` に相当します。古い記事のとおり `shadow` と書くと、意図より強い影になります。本書（v4）では明示的に段階名を付けてください。
...
* Tailwind CSS Docs — Filter: https://tailwindcss.com/docs/filter
