...
> 本当に分けるべきなのは「HTML と CSS」ではなく、<strong>「再利用できないもの」と「再利用できるもの」</strong>だ。
...
Tailwind では、CSS の量は<strong>「使っているユーティリティの種類数」</strong>でほぼ頭打ちになります。`p-4` を 1 か所で使おうと 1,000 か所で使おうと、生成される CSS は `.p-4 { padding: 1rem; }` の 1 つだけだからです。新しいページを 100 個追加しても、すでに使っているユーティリティの組み合わせで作る限り、CSS はほとんど増えません。
