...
- 半角英数字・英単語と日本語の間には、半角スペースを 1 つ入れる。
- このルールは本文だけでなく、見出し、キャプション、`manuscript/SUMMARY.md` の目次表記にも適用する。
- `Tailwind CSSは` ではなく `Tailwind CSS は`、`Utility Firstという` ではなく `Utility First という` とする。
- ただし `第8章` `第1部` `付録A` のような日本語の番号ラベル内では空けない。`第 8 章` `第 1 部` `付録 A` にはしない。
