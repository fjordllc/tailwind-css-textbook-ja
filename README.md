...
| [`scripts/`](./scripts/) | ビルド・デプロイ用スクリプト（Cloudflare Pages 用のビルドスクリプトなど）。 |
...
本書は **Cloudflare Pages** で公開します。Git 連携により、`main` への push で自動ビルド・自動デプロイされます。ビルドには [`scripts/cloudflare-build.sh`](./scripts/cloudflare-build.sh) を使い、ビルド環境へ mdBook を取得してから `mdbook build` を実行します。

ダッシュボードでの初回設定（Build command / 出力ディレクトリなど）や更新手順は [`DEPLOY.md`](./DEPLOY.md) を参照してください。
