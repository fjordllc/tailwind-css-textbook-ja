# FBC Press: Tailwind CSS ― 仕組みと思想から理解する（執筆プロジェクト）
...
本文の構成・執筆・レビュー・ブラウザ閲覧用の mdBook 構成まで一通り整えています。詳細な設計や検討の記録は [`OUTLINE.md`](./OUTLINE.md) と [`REVIEW_NOTES.md`](./REVIEW_NOTES.md) を参照してください。
...
| [`theme/tailwind-css-textbook.css`](./theme/tailwind-css-textbook.css) | mdBook の追加 CSS。日本語テーマ `mdbook-book-jp` を読み込む薄いラッパー（`@import "./mdbook-book-jp.css";`）。本書固有の色・余白などを上書きしたいときの拡張点です。 | ✅ 作成済み |
| [`theme/mdbook-book-core.css`](./theme/mdbook-book-core.css) / [`theme/mdbook-book-jp.css`](./theme/mdbook-book-jp.css) | 日本語テーマ **[mdbook-book-jp](https://github.com/machida/mdbook-book-jp)（別リポジトリ）** から導入した CSS（共通レイアウト層＋日本語タイポグラフィ層）。`install.sh` で配置したコピーなので直接編集せず、更新はテーマ側で行います。 | ✅ 導入済み |
...
- **共通レイアウト層**（`mdbook-book-core.css`）と **日本語タイポグラフィ層**（`mdbook-book-jp.css`）の 2 層構成で、他の日本語 mdBook 書籍にも使い回せます。
- このリポジトリの `theme/mdbook-book-core.css` / `theme/mdbook-book-jp.css` は、そのテーマから**導入したコピー**です。直接編集はせず、本書固有の調整は薄いラッパー `theme/tailwind-css-textbook.css` に書きます。
- `book.toml` の `additional-css` はこのラッパーを指しており、ラッパーが `mdbook-book-jp.css`（さらに内部で `mdbook-book-core.css`）を読み込みます。
